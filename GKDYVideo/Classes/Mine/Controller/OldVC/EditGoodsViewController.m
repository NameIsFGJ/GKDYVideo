...
@interface EditGoodsViewController ()<TZImagePickerControllerDelegate>
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title  = @"编辑宝贝";
...
- (void)networking
{
    [GoodsDetailNetworking postGoodsDetailWithGoodsID:self.goodsID completion:^(NSDictionary * _Nonnull dic, NSError * _Nonnull error) {
       
        // 名称
        self.titleTextField.text = dic[@"name"];
        // 描述
        self.descTextField.text = dic[@"desc"];
        // 价格
        //_priceView.infoTextField
        self.priceView.infoTextField.text = dic[@"price"];
        // 库存
        self.countView.infoTextField.text = dic[@"count"];
        // 分类
